...
export default function Home() {
  ...
  const messagesQuery = trpc.chat.messages.useQuery(messageInput, { enabled: selectedConversationId !== null });
      ...
      <header className="container flex items-center justify-between py-5">
        ...
        {isAuthenticated ? (
          ...
          <Button className="rounded-full px-5" onClick={() => startLogin()}>
            <LockKeyhole className="mr-2 size-3.5" />进入工作台
          </Button>
        )}
      </header>
...
function ChatWorkspace({ isAuthenticated, conversations, conversationsLoading, selectedConversationId, selectedTitle, messages, messagesLoading, creating, sending, onCreate, onSelect, onSend }: {
...
function EmptyChat({ onCreate }: { onCreate: (scenario: Scenario, title: string, firstMessage?: string) => void }) {
  return <div className="flex min-h-[620px] flex-col justify-between p-7 sm:p-10"><div className="max-w-xl"><span className="mb-5 flex size-12 items-center justify-center rounded-2xl bg-primary/10 text-primary"><HeartHandshake className="size-6" /></span><p className="text-xs tracking-[0.2em] text-primary">从一个具体情境开始</p><h2 className="mt-3 font-display text-3xl leading-tight">不急着被说服，<br />先把你的故事讲完整。</h2><p className="mt-5 text-sm leading-7 text-muted-foreground">选择一个快捷开场，或新建一段对话。助手会把事实、感受、推测与可验证的行动分开讨论。</p></div><div className="grid gap-3 sm:grid-cols-3">{SCENARIOS.map((scenario) => <button key={scenario.key} onClick={() => onCreate(scenario.key, scenario.title, scenario.prompt)} className="group rounded-2xl border border-border bg-background p-4 text-left transition-all hover:-translate-y-0.5 hover:border-primary/35 hover:shadow-md"><span className="text-[10px] tracking-[0.16em] text-primary">{scenario.eyebrow}</span><span className="mt-2 block text-sm leading-6">{scenario.title}</span><ChevronRight className="mt-4 size-4 text-muted-foreground transition-transform group-hover:translate-x-1 group-hover:text-primary" /></button>)}</div></div>;
}
